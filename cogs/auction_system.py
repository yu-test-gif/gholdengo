...
class AuctionSystem(commands.Cog):
    ...
    async def reset_auction(self, interaction: discord.Interaction):
        ...
        if not member or not self.is_whitelisted(member):
            await interaction.response.send_message(
                "❌ Only whitelisted can reset auction.", ephemeral=True
            )
            return
    ...
    @app_commands.command(name="auc_register", description="Register a member and give starting coins")
    @app_commands.guilds(discord.Object(id=DEFAULT_GUILD_ID))
    async def auc_register(self, interaction: discord.Interaction, member: discord.Member):
        guild = interaction.guild
        if guild is None:
            await interaction.response.send_message(
                "❌ This command can only be used in a server.", ephemeral=True
        ...
        invoking_member = guild.get_member(interaction.user.id)
        if not invoking_member or not self.is_whitelisted(invoking_member):
            await interaction.response.send_message(
                "❌ Only whitelisted can register members.", ephemeral=True
            )
            return

        uid = str(member.id)
        if uid in self.data["coins"]:
            await interaction.response.send_message(
                f"⚠️ {member.display_name} is already registered.", ephemeral=True
            )
            return

        self.data["coins"][uid] = STARTING_COINS
        self.data["inventory"][uid] = []
        save_data(self.data)
        await interaction.response.send_message(
            f"✅ Registered {member.display_name} with {STARTING_COINS} coins!"
        )
